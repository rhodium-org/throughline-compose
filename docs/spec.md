...
<!-- tl:item SR-0007 -->
**SR-0007 — Union-aware target display in composed documents** — `system_requirement`, status `ratified`

> When tl-compose docs runs over a project that declares sources, it shall inject the consumer's documents over the local consumer graph — so counts, tables and matrix rows are byte-identical to core tl docs — while resolving each tl:matrix target cell through a resolver backed by the composed union. A relationship cell whose target is a namespace-qualified reference to a borrowed clause shall then render that clause's own attribute (for example its source_ref) via the SR-0110 target-display suffix, instead of a namespace-qualified UID the reader cannot look up. Target liveness and attribute lookup for a borrowed target shall read the loaded source for that namespace; an unqualified local target shall fall through to the consumer graph, so behaviour over local links is identical to core tl docs. When the project declares no sources, tl-compose docs shall be a pure pass-through to core tl docs.

*Rationale:* Referring to a borrowed clause by its namespace-qualified IDD UID gives the reader a token they cannot resolve against the upstream framework; the useful reference is the framework's own number. Core throughline exposes exactly the seam needed — an optional target resolver on document injection (SR-0110) — so composition supplies a union-backed resolver rather than reimplementing injection, keeping composition a thin resolution step over the unchanged core (SR-0004) and preserving the strict superset (SR-0003).

**origin**: hybrid · **priority**: should · **verification**: test · **ratified_by**: Henry Grech-Cini
<!-- tl:end -->

<!-- tl:item SR-0008 -->
**SR-0008 — Audit citations affected by a source edition change** — `system_requirement`, status `proposed`

> When a source's pinned edition `ref` changes, the identity of a borrowed item is not guaranteed to survive — an edition commonly keeps its own UID space, so one UID (`asvs:SR-0099`, say) can denote an entirely different clause before and after the change. A structural resolve still succeeds, so `tl-compose check` stays green while every affected citation now points at the wrong target. tl-compose shall offer an audit that, given the old and new refs of a changed source, reports for each existing citation into that source whether the target still exists, whether the target's identifying attribute (its `source_ref` clause number) changed, and whether the target's normative text changed materially. The audit shall classify each citation as unchanged, moved (same meaning, new number), changed (review needed), or withdrawn (no successor), so the composer re-points by meaning rather than trusting a structurally-valid resolution. It shall be read-only and shall not rewrite citations.

*Rationale:* The motivating case is OWASP ASVS v4.0.3 to v5.0.0, a full renumber where `asvs:SR-0099` moves from a session-token rule to a file-path rule; a bare `ref` bump passes every gate yet silently rebinds all citations. `tl check` cannot catch this because the graph stays structurally sound — the failure is one of meaning, not shape, the same blind spot that motivates human ratification elsewhere and the silent-wrong outcome UR-0002 forbids. Surfacing the affected citations turns a repo-wide mis-binding into an explicit, reviewable list, and makes an edition upgrade — an inevitable, recurring event as standards evolve — a controlled migration rather than a leap of faith. It stays firmly on the compose side of NG-0001 because it reasons only over graph data the composer already resolves — two pinned editions of one source — and adds no new detail of its own.
...
| UR-0003 | A source is referenced by origin and pinned to an edition | SR-0006, SR-0008 |
<!-- tl:end -->
